	...
	/*    */ {
		...
		/*    */ {
			...
			/* 45 */ if (!p.hasPermission("kitpvp.kit.monk")) {
				/* 46 */ p.sendMessage(String.valueOf(this.main.getConfig().getString("Prefix").replace("&", "§"))
			...
			/* 68 */ p.sendMessage(String.valueOf(this.main.getConfig().getString("Prefix").replace("&", "§"))
					+ this.main.getConfig().getString("Message.Kit").replaceAll("%kit%", "Monk").replace("&", "§"));
			/*    */
			...
			/* 75 */ for (int i = 0; i <= 34; i++) {
				...
				/* 77 */ me.RafaelAulerDeMeloAraujo.TitleAPI.TitleAPI.sendTitle(p, Integer.valueOf(20),
						...
						this.main.getConfig().getString("Title.KitSubTitle").replaceAll("%kit%", "Monk"));
				/*    */ }
			KPPvP.give(p);
			/*    */ }
		/*    */
