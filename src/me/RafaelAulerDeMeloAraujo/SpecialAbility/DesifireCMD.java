	...
	/*    */ {
		...
		/*    */ {
			...
			/* 40 */ if (Habilidade.ContainsAbility(p)) {
				...
				/* 42 */ p.playSound(p.getLocation(), Sound.valueOf(this.main.getConfig().getString("Sound.KitUse")),
						1.0F, 1.0F);
				/* 43 */ return true;
				/*    */ }
			...
			/* 83 */ p.sendMessage(
					String.valueOf(this.main.getConfig().getString("Prefix").replace("&", "§")) + this.main.getConfig()
							.getString("Message.Kit").replaceAll("%kit%", "Deshfire").replace("&", "§"));
			/* 84 */ Habilidade.setAbility(p, "Deshfire");
			/*    */ RTP.TeleportArenaRandom(p);
			/*    */
			/* 88 */ for (int i = 0; i <= 34; i++) {
				/* 89 */ p.getInventory().addItem(new ItemStack[] { sopa });
				/* 90 */ p.playSound(p.getLocation(), Sound.valueOf(this.main.getConfig().getString("Sound.KitUse")),
						1.0F, 1.0F);
				/* 91 */ TitleAPI.sendTitle(p, Integer.valueOf(20), Integer.valueOf(60), Integer.valueOf(20),
						this.main.getConfig().getString("Title.KitTitle"),
						this.main.getConfig().getString("Title.KitSubTitle").replaceAll("%kit%", "Deshfire"));
				/*    */ }
			KPPvP.give(p);
			/*    */ }
		/*    */
 ...
 * (1).jar!\me\RafaelAulerDeMeloAraujo\SpecialAbility\DesifireCMD.class Java
