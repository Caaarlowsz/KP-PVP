...
/*    */ {
	...
	/*    */ {
		...
		/*    */ {
			...
			/*    */ {
				/* 36 */ p.sendMessage(String.valueOf(this.main.getConfig().getString("Prefix").replace("&", "§"))
			...
			/* 46 */ if (Habilidade.ContainsAbility(p)) {
				/* 47 */ p.sendMessage(String.valueOf(this.main.getConfig().getString("Prefix").replace("&", "§"))
						+ this.main.getConfig().getString("Message.KitUse").replace("&", "§"));
				/* 48 */ p.playSound(p.getLocation(), Sound.valueOf(this.main.getConfig().getString("Sound.KitUse")),
						1.0F, 1.0F);
				/* 49 */ return true;
				/*    */ }
			...
			/*    */
			/* 78 */ p.sendMessage(String.valueOf(this.main.getConfig().getString("Prefix").replace("&", "§"))
					+ this.main.getConfig().getString("Message.Kit").replaceAll("%kit%", "Thor").replace("&", "§"));
			...
			/* 85 */ for (int i = 0; i <= 34; i++) {
				...
				/* 87 */ p.playSound(p.getLocation(), Sound.valueOf(this.main.getConfig().getString("Sound.KitUse")),
						1.0F, 1.0F);
				/* 88 */ me.RafaelAulerDeMeloAraujo.TitleAPI.TitleAPI.sendTitle(p, Integer.valueOf(20),
 ...
 * (1).jar!\me\RafaelAulerDeMeloAraujo\SpecialAbility\ThorKITCOMMAND.class Java
