	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("tp")) {
			...
			if (args.length == 1) {
				...
				Bukkit.broadcast(API.NomeServer + "§a" + player.getName() + ChatColor.GRAY + " se teleportou para: §a"
						+ target.getName(), "kitpvp.staff");
				return true;
			}
			if (args.length > 1) {
				player.sendMessage(API.NomeServer + "Use /tp [player] para ir ate um player!");
			...
			if (args.length == 1) {
				...
				Bukkit.broadcast(API.NomeServer + "§a" + player.getName() + ChatColor.GRAY + " puxou o jogador:"
						+ target.getName() + " ate ele", "kitpvp.staff");
				return true;
			}
			if (args.length > 1) {
				player.sendMessage(API.NomeServer + "Use /tp [player] para ir ate um player!");
