	...
	/*    */ {
		...
		/*    */ {
			/* 30 */ if (!sender.hasPermission("kitpvp.reload")) {
				/* 31 */ p.sendMessage(String.valueOf(this.main.getConfig().getString("Prefix").replace("&", "§"))
						+ this.main.getConfig().getString("Permission").replace("&", "§").replaceAll("%permisson%",
								label));
				/* 32 */ p.playSound(p.getLocation(),
						org.bukkit.Sound.valueOf(this.main.getConfig().getString("Sound.NoPermissionMessage")), 1.0F,
						1.0F);
				/* 33 */ return true;
				/*    */ }
			/*    */
 ...
 * (1).jar!\me\RafaelAulerDeMeloAraujo\main\Reload.class Java compiler version:
