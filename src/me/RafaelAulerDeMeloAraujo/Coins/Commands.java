		...
		if (cmd.getName().equalsIgnoreCase("givecoins")) {
			if (!sender.hasPermission("kitpvp.coins")) {
				...
				if (isNumeric(args[1])) {
					...
					sender.sendMessage("§eYou give the player " + target.getName() + "" + coins + "§bCoins");
					target.sendMessage("§eYou receive " + coins + " §bCoins");
 ...
 * (1).jar!\me\RafaelAulerDeMeloAraujo\Coins\Commands.class Java compiler
